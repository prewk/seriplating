...
use Mockery;
use Prewk\Seriplating\BidirectionalTemplates\ContainerTemplate;
...
class HierarchicalTemplateIntegrationTest extends SeriplatingTestCase
{
    ...
    public function test_hierarchical_serialization_with_container_template()
    {
        $t = new Seriplater(new Rule);
        $idResolver = new IdResolver;
        $idFactory = new IdFactory;
        ...
        $topTemplate = new ContainerTemplate($idFactory, $idResolver, [
            "id" => $t->id("tops"),
            "foos" => $t->hasMany("foos"),
            "bars" => $t->hasMany("bars"),
        ]);
